...
- The WebApp includes an Anti-collision Scan workflow with the standard Field/Cluster/Well/WellBore/Trajectory hierarchy, planned/actual and definitive filtering, selectable octree candidates, sparse or all-reference-depth separation-factor tables, and grouped interactive profiles with measured depth positive downward. Both octree candidate discovery and separation-factor calculation run as server-side jobs with regularly polled stage/progress updates, so a multi-minute operation does not hold one HTTP request open.

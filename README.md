...
- MCP tools are generated from 118 non-statistics controller actions; the usage-statistics controller is intentionally excluded. A `ping` tool is also available. Tool metadata provides operation-specific workflow guidance and complete nested JSON input schemas with UUID formats, enum values, defaults, chunk-index constraints, identity/feature assignments, and SI units.

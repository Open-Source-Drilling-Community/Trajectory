...
- MCP tools are generated from 120 non-statistics controller actions; the usage-statistics controller is intentionally excluded. A `ping` tool is also available. Every tool publishes a title, strict input and success-output schemas, safety annotations, and operation-specific workflow guidance. Schemas enforce non-empty UUIDs, reject unknown arguments, constrain usable backup/restore policies, and document nested models, chunk indexes, identity/feature assignments, and SI units. Successful calls provide structured JSON and text fallback; failures use stable sanitized MCP error envelopes.
